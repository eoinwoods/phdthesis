...
In addition, a secondary area of validation we wish to perform is to confirm that \emph{CPU is a valid proxy for overall resource usage} when performing energy allocation calculations.  For this validation we focus on how CPU usage varies for disk IO intensive workloads.

Throughout this validation testing we aim to achieve consistency of results to within 5\% tolerance.  Long experience has taught us that a high degree of consistency is very difficult to achieve in any performance or resource utilisation testing due to the number of factors that can affect the results of a test on a modern multi-cpu, multi-core server machine.  Our experience in previous testing work is that a 5\% tolerance in most cases is lowest degree of variation we are likely to achieve, being equivalent to equality plus the variation caused by factors outside our control.
...
\section{Validating the Allocation Calculation}
...
This result is well within our target result tolerance and so validates the Apollo proof-of-concept implementation's accuracy.

\section{Validating Allocation Consistency}
...
\section{Validating Allocation Scenarios}
