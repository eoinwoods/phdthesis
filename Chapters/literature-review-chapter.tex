...
Two key characteristics of the ADLs we wanted to understand were the application domains that they targeted and the degree to which they had been applied.
...
\item[ADL.RQ2] \emph{What application domains is each ADL targeted at?}
Two thirds of the ADLs we studied are not specialised for a specific technical or business application domain.  Those that are specialised for a specific domain are specialised for embedded systems (11\%), distributed systems (8\%), highly concurrent systems (6\%) and SOA (6\%).  There are a small number of other ADLs (10\%) aimed at other specialised technical domains like stream processing and event driven systems. \\
General ADLs without a specific domain specialisation are more widely applicable that ones with a specific focus, and given the costs of developing, promoting and supporting them, it is understandable that many researchers aim for generality.  However, the trade-off implied by generality is a lack of focus on the specific requirements of a specific context, which means that the more general an ADLs is, the more difficult it is likely to be to apply initially, as it will require more tailoring or consideration of how to apply it, than a more specific language.  That said, recent research \cite{malavolta2013-industryadlneeds} suggests that practitioners prefer generic rather than more domain focused languages, so while there may be higher initial adoption costs or effort, practitioners clearly value the flexibility of generic languages.  The fact that many ADLs are generic may therefore be a positive factor for adoption.

\item[ADL.RQ3] \emph{Does the ADL support first class connectors?}
Of the ADLs in this survey more than half of them (33 or 65\%) provide first class connectors in the language.  The remaining 18 (35\%) do not.  We viewed this as a positive conclusion for the possible application of these ADLs to industrial problems.  First class connectors provide a degree of sophistication in architectural description which can provide new facilities that could motivate the adoption of ADLs in industry. 

\item[ADL.RQ4] \emph{Does the ADL support first class configuration?}
Of the ADLs in this survey, most of them (41 or 80\%) provide first class architectural configuration constructs in the language, while 10 (20\%) do not.
First class architectural configuration is a requirement for supporting architecture level reuse, which again could be a possible motivation for applying an ADL in an industrial context.  Therefore, the fact that most of the ADLs allow this suggests that they could provide some innovative new options for an industrial software architect.

\item[ADL.RQ5] \emph{Does the ADL provide structuring mechanisms for large ADs?}
...
\item[ADL.RQ7] \emph{Can system qualities or quality requirements be captured in the ADL?}
...
\item[ADL.RQ8] \emph{Were prototype or production quality tools developed with the ADL?}
...
\item[ADL.RQ9] \emph{Has the ADL been applied to non-trivial problems outside the group of people who created it?}
