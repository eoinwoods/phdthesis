...
The specific contribution of this work is to design an approach to measuring resource usage for request processing in a distributed application in such a way that an energy model can be used to estimate the energy usage of specific requests made to the application.
...
\begin{itemize}
\item The business logic in the system is implemented as a group of small, focused services, each performing one task, typically implementing a ?bounded context? in Domain Driven Design terms.
...
We designed microservice-based systems typically share other important characteristics, such as independent build, test and deploy for each service, and interfaces defined using machine readable formats such as Swagger [REF] and RAML [REF] but these other characteristics and not significant in the context of this work, and so we do not assume that they are present.
