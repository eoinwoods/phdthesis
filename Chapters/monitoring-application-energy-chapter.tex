...
Given these useful aliases we can now define some key subsidiary functions
...
Given $a$ which is an architectural element address and $st$ and $et$ which are the start and end times of a period respectively
\begin{equation}
cpuHostUsage(a, st, et) :: TODO
\end{equation}

Given $a$, $st$ and $et$ as defined above
\begin{equation}
energy(a, st, et) :: TODO
\end{equation}


\textbf{TODO} - define cpuUsage(), cpuHostUsage() and energy()

Given these supporting definitions we can now define the specification for the energy estmation process to be

\begin{equation}
E_{t} = \sum_{i=1}^{\#t.S}(\frac{cpuUsage(t.S[i])}{cpuHostUsage(a, st, et)} ) \times energy(a, st, et)
\end{equation}
