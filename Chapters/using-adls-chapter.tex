...
\section{Related Work}
  ...
  While ADLs originated in academia such as ACME [13]/ADML, ABACUS [14], Aesop [15], UniCon [16], Wright [17], GEN-VOCA [18], π-ADL [3], Rapide [19], SADL [20], xADL [21], ADLARS [22], ALI [23, 24], ArchiMate [25] and ByADL [1], to name a few, all exhibit novel approaches to architecture description, from support for interchange and interoperability to advanced architectural analysis capabilities, the vast majority tend to be vertically optimized, limiting their attractiveness in many industrial projects.  
...
* System - the entire information system being described, which is a conceptual structure, composed of a number of interconnected subsystems that collectively provide its behaviour and qualities.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{Figures/adls-figure1.pdf}
...
7	A CASE STUDY OF THE APPROACH IN USE
...
B	Example Scenario - Generate Order List
...
The Job Processor sends a status message indicating that new order lists exist, which is routed to the UI Server
...
“ordermgmt” and “pmgmt” schemas

The UI Server accesses the database to get the new portfolio state and associated order list state
...
The GUI calls the UI Server for a status update and gets details of the new order list in return
...
The GUI makes an RPC call to the UI Server to indicate that the order list should be traded
...
The UI Server creates a message to request the order list to be traded (including the list of orders) which is routed to the Order Gateway
...
The Order Gateway sends the orders to an external trading system and receives status updates in return as the orders are executed
...
[3]	F. Oquendo, "π-ADL: an Architecture Description Language based on the higher-order typed π-calculus for specifying dynamic and mobile software architectures," ACM SIGSOFT Software Engineering Notes, vol. 29, pp. 1-14, 2004.
